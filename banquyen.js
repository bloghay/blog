(function (a, b) {
	...
	function b$(a, b) {
		...
		for (; f < g; f++) {
			...
			b ? (!e[f] && c.style.display === "none" && (c.style.display = ""), c.style.display === "" && bZ(c) && (e[f] = p._data(c, "olddisplay", cc(c.nodeName)))) : (d = dd(c, "display"), !e[f] && d !== "none" && p._data(c, "olddisplay", d))
		}
		for (f = 0; f < g; f++) {
			c = a[f];
			if (!c.style) continue;
	...
	function ca(a, b, c, d) {
		...
		d ? (c === "content" && (f -= parseFloat(dd(a, "padding" + bV[e])) || 0), c !== "margin" && (f -= parseFloat(dd(a, "border" + bV[e] + "Width")) || 0)) : (f += parseFloat(dd(a, "padding" + bV[e])) || 0, c !== "padding" && (f += parseFloat(dd(a, "border" + bV[e] + "Width")) || 0));
		return f
	}
	function cb(a, b, c) {
	...
	function (a, b) {
		...
		function bn(a, b) {
			...
			g = function (h, i, j, k, m) {
				...
				if (d && u !== s) {
					...
					w.apply(k, q),
					y && !h && q.length > 0 && s + b.length > 1 && db.uniqueSort(k)
				}
				return y && (t = B, l = z),
				x
			};
		...
		e = db.selectors = {
			...
			preFilter: {
				...
				CHILD: function (a) {
					...
					a[1] === "nth" ? (a[2] || db.error(a[0]), a[3] = +(a[3] ? a[4] + (a[5] || 1) : 2 * (a[2] === "even" || a[2] === "odd")), a[4] = +(a[6] + a[7] || a[2] === "odd")) : a[2] && db.error(a[0]),
					a
				},
				PSEUDO: function (a) {
					var b, c;
		...
		p.unique = db.uniqueSort,
		p.text = db.getText,
		p.isXMLDoc = db.isXML,
		p.contains = db.contains
	} (a);
	var db = /Until$/,
	...
	p.fn.extend({
		...
		append: function () {
			return this.domManip(arguments, !0, function (a) { (this.nodeType === 1 || this.nodeType === 11) && this.appendChild(a)
			})
		},
		prepend: function () {
			return this.domManip(arguments, !0, function (a) { (this.nodeType === 1 || this.nodeType === 11) && this.insertBefore(a, this.firstChild)
			})
		},
		before: function () {
		...
		remove: function (a, b) {
			var c, d = 0;
			for (;
			(c = this[d]) != null; d++) if (!a || p.filter(a, [c]).length) ! b && c.nodeType === 1 && (p.cleanData(c.getElementsByTagName("*")), p.cleanData([c])),
	...
	p.extend({
		...
		clean: function (a, b, c, d) {
			...
			if (c) {
				...
				for (f = 0;
				(h = t[f]) != null; f++) if (!p.nodeName(h, "script") || !q(h)) c.appendChild(h),
	...
	p.extend({
		...
		ajax: function (a, c) {
			...
			l.url = ((a || l.url) + "").replace(cl, "").replace(cp, df[1] + "//"),
			l.dataTypes = p.trim(l.dataType || "*").toLowerCase().split(s),
			l.crossDomain == null && (i = ct.exec(l.url.toLowerCase()) || !1, l.crossDomain = i && i.join(":") + (i[3] ? "": i[1] === "http:" ? 80 : 443) !== df.join(":") + (df[3] ? "": df[1] === "http:" ? 80 : 443)),
	...
	p.ajaxPrefilter("json jsonp", function (c, d, e) {
		...
		e.always(function () {
			a[f] = g,
			c[f] && (c.jsonpCallback = d.jsonpCallback, dg.push(f)),
		...
		if (!c.crossDomain || p.support.cors) {
			...
			return {
				send: function (e, f) {
					...
					d = function (a, e) {
						...
						try {
							if (d && (e || i.readyState === 4)) {
								d = b,
								g && (i.onreadystatechange = p.noop, cJ && delete dh[g]);
					...
					c.async ? i.readyState === 4 ? setTimeout(d, 0) : (g = ++cK, cJ && (dh || (dh = {},
					p(a).unload(cJ)), dh[g] = d), i.onreadystatechange = d) : d()
				},
				abort: function () {
					d && d(0, 1)
				}
...
$(document).ready(function () {
	$('#hdcopyright').html('<a id="hdcopyright" style="color:auto!important;position:relative!important;display:inline!important;visibility:visible!important;height:auto!important;width:auto!important;" alt="Template được thiết kế bởi Nghĩa anh" href="http://www.bloghay.xyz/" target="_blank" title="Template được thiết kế bởi Nghĩa anh">Hay</a>');
	setInterval(function () {
		if (!$('#hdcopyright:visible').length) window.location.href = 'http://www.bloghay.xyz/'
	},
	1000)
})